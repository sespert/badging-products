...
selectIdsTour = () => {
    ...
    var tourArr = [];
    nightsArr.map(elem => tourArr.push(idArr[elem]));
...
findPrices = () => {
    ...
    //Save an array with only those elements that fit the promotion criteria
    var indexPrice = [];
    numArr.map((elem, i) => elem < 250 ? indexPrice.push(i) : null);
...
selectIdsDiscount = () => {    
    ...
    var discArr = [];
    pricesArr.map((elem,i) => discArr.push(idArr[i]));
...
finalComparison = () => {
    ...
    tourArr.map(elem => newPriceArr.includes(elem) ? newPriceArr = arrayRemove(newPriceArr, elem) : null);
...
addBadges = () => {
    ...
    //EACH BADGE DIFFERENT ACCORDING TO MOBILE RESPONSIVENESS
    ...
    tourArr.map(elem => $("#" + elem).find("figure").html(tourSpan));
    priceArr.map(elem => $("#" + elem).find("figure").html(discSpan));
}
